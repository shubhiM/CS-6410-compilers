...
let run_no_vg (program_name : string) : (string, string) result =
...
let run_vg (program_name : string) : (string, string) result =
  let (rstdout, rstdout_name, rstderr, rstderr_name, rstdin) = make_tmpfiles "run" in
...
let run_asm asm_string (out : string) (runner : string -> (string, string) result) =
  ...
  let try_running = match status with
    ...
    | WEXITED n ->
       Error(sprintf "Finished with exit-code %d while building %s:\n%s" n out (string_of_file bstderr_name))
    | WSIGNALED n ->
...
let run (p : sourcespan program) (out : string) (runner : string -> (string, string) result) : (string, string) result =
  let maybe_asm_string = compile_to_string (Ok(p, [])) in
  match maybe_asm_string with
  | Error(errs, _) -> Error(ExtString.String.join "\n" (print_errors errs))
  | Ok(asm_string, _) ->
     run_asm asm_string out runner

let run_anf p out runner =
