...
let tag (e : 'a expr) : tag expr =
  let rec tag_helper (e : 'a expr) (curr : tag) =
    ...
    | ELet(binds, body, _) ->
      ...
      let tagged_binds, next_tag = List.fold_left
        (
          fun ((t_bindings : tag bind list), (n_tag : tag)) ((v, exp, _): 'a bind) ->
  ...
  in
  let (tagged, _) = tag_helper e 1 in tagged
;;
...
let rec anf_helper (e: tag expr) : (unit expr * (string * unit expr) list) =
  ...
  |EPrim2(op, left, right, tag) ->
   ...
   |ELet(bindings, body, tag) ->
    ...
    let anfed_bindings, context_of_anfed_bindings = List.fold_left
    (
      fun ((a_binds : unit bind list), (context : (string * unit expr) list)) ((v, exp, tg) : tag bind) ->
          let b_ans, b_contxt = anf_helper exp in
          (a_binds @ [(v, b_ans, ())], context @ b_contxt)
    )
    ([], [])
    bindings
    in
   ...
   |EIf(cond, thn, els, tag) ->
    failwith "Not implemented if expressions yet"
