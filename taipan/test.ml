...
let tvg name program expected =
  name >:: test_run_valgrind program name expected
...
(* shadow errors *)
let prog_err_43 =
  "let x = true, y = false in let x = y  in if y: true else: false"
...
let prog_err_48 =
  ...
  \                        if (let x = a, y = b in (let z = (x && y), w = (x \
   || y) in let x = isbool(z) && isnum(w) in x)):\n\
  \                          (a && b)\n\
  \                          else:\n\
  \                          (a || b)"

(* foo2 is not in scope *)
let prog_err_50 =
  "def foo1(x, y):\n\
  \                         foo2(x, y)\n\
  \                     def foo2(x, y):\n\
...
(* function overloading is not supported, foo1 should give duplicate error *)
let prog_err_51 =
  "def foo1(x, y):\n\
  \                         x + y\n\
  \                     and def foo2(x, y):\n\
...
(* duplicate fun name in a different group *)
let prog_err_52 =
  "def foo1(x, y):\n\
  \                         x + y\n\
  \                     and def foo2(x, y):\n\
...
let prog_3 =
  ...
  \                (let x = 2, y = 3, foo1 = x, foo2 = y in foo1(x, y) + \
   (foo1 * foo2)) +\n\
...
let multiple_well_formedness_errs =
  ...
  ; (* Tests to check new function scoping rules for mutually recursive functions *)
    te "t_err_50" prog_err_50 "The function name foo2, used at"
  ; te "t_err_51" prog_err_51 "The function name foo1, redefined at"
  ; te "t_err_52" prog_err_52 "The function name foo2, redefined at" ]
...
let let_tests =
  ...
  ; t "let_14"
      {| let x = 1 in
                    let y = if ((x - 1) > 0) : 0 else: x in x |}
      "1"
  ; t "let_15"
      {| let x = 2 in
                    let y = if ((x - 1) == 1): x else: 0 in x |}
      "2"
  ; t "let_17" {| let x = (let y = 2 in y) in x |} "2"
  ; t "let_18" {| let x = 2 * 2 + 1, y = x * 2, z = y - (x * y) in z |} "-40"
  ; t "let_19" {| let x = add1(4), y = x * add1(1), z = y - (x * y) in z |}
      "-40" ]
...
let if_tests =
  ...
  ; t "if_21" {| if true: (let x = false in let y = true in x || y) else: 2 |}
      "true"
  ; t "if_22" {| if (let x = true in x): (let x = 2 in x) else: 0 |} "2"
  ; t "if_23"
      {| if ((let x = 2 in x) - 2) > 0: 0
                 else: (if ((let y = 2 in y * 2) - 4) > 0: 0 else: 10) |}
      "10"
  ; t "if_24"
      {| (if ((let x = 2 in x) - 2) < 0: 0
                  else: (if (let y = false in y): true else: false)) && true |}
      "false"
  ; t "if_25"
      {| if (let x = false, y = true in x || y): (let x = 1, y = 2 in x * y) else: 1 |}
      "2" ]
...
let arith_err_tests =
  ...
  ; te "t_arith_err_17" "1 + 2 * 3 + 4 - 5 * true"
...
let let_and_if_err =
  [ te "if_err_1" {| if 1 < 3 < 4: (4 - 3) * (1 + 2) else: 0 |}
      "comparison expected a number"
  ; te "if_err_2"
      {| if (let x = 1 in x) < (let y = 2 in y) < (let z = 3 in z): true else: false |}
      "comparison expected a number"
  ; te "if_err_3"
      {| if (let x = 1 in x) < (let x = 2 in x) < (let x = 3 in x): true else: false |}
      "comparison expected a number"
  ; te "let_err_1" {| let x = 1, y = 2, z = if x: 3 else: 4 in z |}
      "if expected a boolean" ]

let suite =
  "suite"
  >::: simple_well_formedness_err @ multiple_well_formedness_errs
       @ int_unary_op_tests @ int_binary_op_tests @ bool_tests
       @ int_cmp_op_tests @ if_tests @ let_tests @ arith_err_tests
       @ logical_err_tests @ let_and_if_err @ correct_programs
